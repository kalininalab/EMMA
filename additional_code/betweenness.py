...
# ==============================================
def analyze(B, betweenness, OUTPUT_DIR,df_name):
...
# Set style for Nature journal
def plot_betweenness(path):
    ...
    splits = [("C1f", "enzyme-based"), ("C1e", "small molecule-based"), ("C1", "Label-based"),
              ("C2", "two-dimensional")]
    betweenness_colors = {'Enzyme': '#fc8d62', 'Small molecule': '#8da0cb'}
    ...
    all_betweenness_values = []
    all_kde_max_values = []

    # First, collect all data to set consistent axis limits
    for s in splits:
        betweenness_test = pd.read_csv(join(OUTPUT_DIR, f"betweenness_values_test_{s[0]}_2S.csv"))
        betweenness_train = pd.read_csv(join(OUTPUT_DIR, f"betweenness_values_train_{s[0]}_2S.csv"))
        betweenness_test['Betweenness'] = np.log(betweenness_test['Betweenness'] + 1)
        betweenness_train['Betweenness'] = np.log(betweenness_train['Betweenness'] + 1)
        betweenness_test['Type'] = betweenness_test['Type'].replace('Molecule', 'Small molecule')
        betweenness_train['Type'] = betweenness_train['Type'].replace('Molecule', 'Small molecule')

        # Store for axis limits
        all_betweenness_values.extend(betweenness_train['Betweenness'].tolist())
        all_betweenness_values.extend(betweenness_test['Betweenness'].tolist())
        ...
        for dataset in [betweenness_train, betweenness_test]:
            for node_type in ['Enzyme', 'Small molecule']:
                data_subset = dataset[dataset['Type'] == node_type]['Betweenness']
    ...
    global_box_min = min(all_betweenness_values)
    global_box_max = max(all_betweenness_values) * 1.05

    # Calculate minimum x limit - ensure no negative values
    global_x_min = max(0, min(all_betweenness_values) * 0.9)  # Start at 0 or slightly below min
    ...
    for s in splits:
        ...
        betweenness_train['Dataset'] = 'Train'
        betweenness_test['Dataset'] = 'Test'

        # KDE Plot for Train set - Column 0
        ax_kde_train = fig.add_subplot(gs[row, 0])
        sns.kdeplot(data=betweenness_train, x='Betweenness', hue='Type', fill=True,
                    palette=betweenness_colors, alpha=0.6, linewidth=1,
        ...
        if row == 3:
            ax_kde_train.set_xlabel('BC score (log + 1 scaled)')
        else:
        ...
        sns.kdeplot(data=betweenness_test, x='Betweenness', hue='Type', fill=True,
                    palette=betweenness_colors, alpha=0.6, linewidth=1,
        ...
        # Only show x label for bottom row (row 3)
        if row == 3:
            ax_kde_test.set_xlabel('BC score (log + 1 scaled)')
        else:
        ...
        if row == 0:
            legend_elements = [
                Patch(facecolor=betweenness_colors['Enzyme'], label='Enzyme'),
                Patch(facecolor=betweenness_colors['Small molecule'], label='Small molecule')
            ]
        ...
        sns.boxplot(data=betweenness_train, x='Type', y='Betweenness', hue='Type',
                    palette=betweenness_colors, showfliers=False, width=0.4,
        ...
        ax_box_train.set_ylabel('BC score (log + 1 scaled)')
        ...
        sns.boxplot(data=betweenness_test, x='Type', y='Betweenness', hue='Type',
                    palette=betweenness_colors, showfliers=False, width=0.4,
    ...
    plt.tight_layout()
    output_path = join(OUTPUT_DIR, 'betweenness_analysis_combined.pdf')
