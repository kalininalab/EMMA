...
def extract_substrate(df):
    ...
    sub_km["parameter.startValue"] = (sub_km["parameter.startValue"].astype(float) * 1e9)
    substrate = data.copy()
    ...
    final_data = final_data.groupby(['Uni_SwissProt', 'Substrate'], as_index=False).apply(
        lambda x: x[x['standard_value'].notna()].head(1) if x['standard_value'].notna().any() else x.head(
...
chebi_to_smiles_df = pd.read_pickle(
    join(CURRENT_DIR, "..", "data", "processed_data", "sabio", "sabio_chebi2smiles.pkl"))
...
substrate_df["Evidence"] = "BAA"
substrate_df["activity_comment"] = None
inhibitors_df["Evidence"] = "BAA"
...
substrate_df.to_pickle(join(CURRENT_DIR, "..", "data", "processed_data", "sabio", "10-sabio_enz_sub.pkl"))
inhibitors_df.to_pickle(join(CURRENT_DIR, "..", "data", "processed_data", "sabio", "10-sabio_enz_inh.pkl"))
