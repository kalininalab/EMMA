...
sabio_enz_sub_1 = sabio_enz_sub[
    (sabio_enz_sub['standard_value'].notna()) & (sabio_enz_sub['standard_value'] > 0) & (sabio_enz_sub['standard_value'] <= EnSub_km_nM)]
...
sabio_enz_inh_1 = sabio_enz_inh[
    (sabio_enz_inh['standard_value'].notna()) & (sabio_enz_inh['standard_value'] > 0) & (sabio_enz_inh['standard_value'] <= EnInh_KiIcMic_nM)]
############################################################################
# Brenda Enzyme-Non-Substrate
####################
brenda_enz_sub_1 = brenda_enz_sub[
    (brenda_enz_sub['standard_value'].notna()) & (brenda_enz_sub['standard_value'] > 0) & (brenda_enz_sub['standard_value'] <= EnSub_km_nM)]
...
brenda_enz_inh_1 = brenda_enz_inh[
    (brenda_enz_inh['standard_value'].notna()) & (brenda_enz_inh['standard_value'] > 0) & (brenda_enz_inh['standard_value'] <= EnInh_KiIcMic_nM)]
...
enz_inh_final = pd.concat([add_binding_source(df, binding_value, source) for df, binding_value, source in datasets_inh],
                          axis=0)
enz_inh_final = canonicalize_and_deduplicate_and_len_constraint(enz_inh_final)
...
enz_ni_final = canonicalize_and_deduplicate_and_len_constraint(enz_ni_final)
...
enz_sub_final = pd.concat([add_binding_source(df, binding_value, source) for df, binding_value, source in datasets],
                          axis=0)
enz_sub_final = canonicalize_and_deduplicate_and_len_constraint(enz_sub_final)
print(data_report(enz_sub_final))

##########################################################################
# Save data before down-sampling
####################
df_enz_inh_path = os.path.join(CURRENT_DIR, "..", "data", "processed_data", "df_enz_inh_before_downsampling.pkl")
df_enz_ni_path = os.path.join(CURRENT_DIR, "..", "data", "processed_data", "df_enz_ni_before_downsampling.pkl")
df_enz_sub_path = os.path.join(CURRENT_DIR, "..", "data", "processed_data", "df_enz_sub.pkl")
if os.path.exists(df_enz_inh_path) and os.path.exists(df_enz_ni_path):
    enz_inh_final = pd.read_pickle(df_enz_inh_path)
    enz_ni_final = pd.read_pickle(df_enz_ni_path)
    enz_sub_final = pd.read_pickle(df_enz_sub_path)
else:
    enz_inh_final.to_pickle(df_enz_inh_path)
    enz_ni_final.to_pickle(df_enz_ni_path)
    enz_sub_final.to_pickle(df_enz_sub_path)
...
selected_enz_inh, selected_enz_ni = down_sampling(
    df_inh,
    df_enz_ni,
...
enz_inh_final = enz_inh_final[enz_inh_final['molecule_ID'].isin(selected_enz_inh)]
print(data_report(enz_ni_final))
print(data_report(enz_inh_final))
...
enz_ni = final_dataset.loc[final_dataset['Binding'] == 2]
print(data_report(enz_ni))
molecule_distribution(df_inh, enz_inh_final, "Inhibitors")
print(enz_ni["standard_type"].unique().tolist())
molecule_distribution(df_enz_ni, enz_ni_final, "Non-interacting")
