...
def main(args):
    ...
    if split_method == 'R' and len(split_size) == 2:
        train, test = train_test_split(data, test_size=0.2, random_state=42, stratify=data['Binding'])
        ...
        logging.info(f"Ratio of inh  to sub in train: {round(len(train.loc[train['Binding'] == 0]) / len(train.loc[train['Binding'] == 1]), 3)}")
        logging.info(f"Ratio of inh  to sub in test: {round(len(test.loc[test['Binding'] == 0]) / len(test.loc[test['Binding'] == 1]), 3)}")
        logging.info(f"Ratio of inh  to ni in train: {round(len(train.loc[train['Binding'] == 0]) / len(train.loc[train['Binding'] == 2]), 3)}")
        logging.info(f"Ratio of inh  to ni in test: {round(len(test.loc[test['Binding'] == 0]) / len(test.loc[test['Binding'] == 2]), 3)}")
        logging.info(f"Ratio of sub  to ni in train: {round(len(train.loc[train['Binding'] == 1]) / len(train.loc[train['Binding'] == 2]), 3)}")
        logging.info(f"Ratio of sub  to ni in test: {round(len(test.loc[test['Binding'] == 1]) / len(test.loc[test['Binding'] == 2]), 3)}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--split-method', type=str, required=True,
                        help="The split method should be one of [C2, C1e, C1f, R]")
    ...
    parser.add_argument("--output-path", type=str, required=True, help="Directory where the splits are saved.")
