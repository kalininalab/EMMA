...
def main(args):
    ...
    base_split = args.split_method

    if len(split_size) not in [2, 3]:
        raise ValueError("The split-size argument must be a list of either two or three integers.")
    split_path = join(output_path, f"{base_split}_epsilon{epsilon}_delta{delta}_{len(split_size)}S")
    os.makedirs(split_path, exist_ok=True)
    log_file = os.path.join(split_path,
                            f"Report_{base_split}_ep{epsilon}del{delta}_{len(split_size)}S.log")
    ...
    train_output_file = os.path.join(split_path, f"train_{base_split}_{len(split_size)}S.pkl")
    test_output_file = os.path.join(split_path,
                                    f"test_{base_split}_{len(split_size)}S.pkl")
    val_output_file = os.path.join(split_path, f"val_{base_split}_{len(split_size)}S.pkl")
    final_data_output_file = os.path.join(split_path, f"Final_Dataset_{base_split}_{len(split_size)}S.pkl")
    data['ids'] = ['ID' + str(index) for index in data.index]
    #labels = data["Binding"].unique().tolist()
    results = []
    for label in [0,2,1]:
        ...
        if ratio_mol_to_pro > 1:
            ...
            print(split_tech)
            _, f_splits, _ = datasail_wrapper(split_tech, label_data, split_size, stratification=strat,
            ...
            split_tech = split_method + "f"
            print(split_tech)
            e_splits, _, _ = datasail_wrapper(split_tech, label_data, split_size, stratification=strat,
                                                 epsilon=epsilon, delta=delta)
    ...
    if len(split_size) == 2:
        ...
        logging.info(
            f"Data report after splitting data by {base_split} split method and check for NaN or null cells in the "
            f"data\n{result.to_string()}")
        ...
        logging.info(
            f"Ratio of inhibitors  to substrates in train: {len(train.loc[train['Binding'] == 0]) / len(train.loc[train['Binding'] == 1])}")
        logging.info(
            f"Ratio of inhibitors  to substrates in test: {len(test.loc[test['Binding'] == 0]) / len(test.loc[test['Binding'] == 1])}")
        logging.info(
            f"Ratio of inhibitors  to non-substrates in train: {len(train.loc[train['Binding'] == 0]) / len(train.loc[train['Binding'] == 2])}")
        logging.info(
            f"Ratio of inhibitors  to non-substrates in test: {len(test.loc[test['Binding'] == 0]) / len(test.loc[test['Binding'] == 2])}")

        logging.info(
            f"Ratio of substrates  to non-substrate in train: {len(train.loc[train['Binding'] == 1]) / len(train.loc[train['Binding'] == 2])}")
        logging.info(
            f"Ratio of substrates  to non-substrates in test: {len(test.loc[test['Binding'] == 1]) / len(test.loc[test['Binding'] == 2])}")
        end_time = time.time()
        ...
        logging.info(
            f"Ratio of substrates  to non-substrates in test: {len(test.loc[test['Binding'] == 1]) / len(test.loc[test['Binding'] == 2])}")
        logging.info(
            f"Ratio of inhibitors  to substrates in val: {len(val.loc[val['Binding'] == 0]) / len(val.loc[val['Binding'] == 1])}")
        logging.info(
            f"Ratio of inhibitors  to non-substrate in val: {len(val.loc[val['Binding'] == 0]) / len(val.loc[val['Binding'] == 2])}")
        logging.info(
            f"Ratio of substrates  to non-substrate in val: {len(val.loc[val['Binding'] == 1]) / len(val.loc[val['Binding'] == 2])}")
        end_time = time.time()
        total_time = (end_time - start_time) / 60
        logging.info(f"Time taken for the process: {total_time:.2f} minutes")
