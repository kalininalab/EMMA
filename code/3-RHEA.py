...
df_RHEA_uniprot_go = pd.merge(df_RHEA_uniprot_go, experimental_df, left_on=['Uniprot_ID', "GO_ID"], right_on=['Uniprot ID',"GO Term"], how='inner')
...
#######################################
# source mapping tables: https://chembl.gitbook.io/chembl-interface-documentation/frequently-asked-questions/general-questions
...
chembl2pub['Chebi_ID'] = chembl2pub['Chebi_ID'].apply(lambda x: f'CHEBI:{int(x)}' if pd.notna(x) and str(x).replace('.', '').isdigit() else x)
...
df_RHEA_exploded = df_RHEA_exploded[['Uni_SwissProt', 'molecule_ID', 'Protein_Sequence', 'SMILES','EC_ID','Evidence','activity_comment']]
