...
for term in Lines.split('[Term]\n')[1:]:
    ...
    if any(start in definition for start in starters):
        ...
        df = pd.concat([df, pd.DataFrame({
            ...
            "RHEA ID": [RHEA_ID], "Substrates": [substrates], "MetaCyc ID":MetaCyc_ID,"Reactome ID":Reactome_ID,"EC_ID":EC_ID, "KEGG_ID":KEGG_ID
        })], ignore_index=True)
...
df_obo_uniprot = pd.merge(df_obo_uniprot, experimental_df, left_on=['Uniprot_ID', "GO ID"], right_on=['Uniprot ID',"GO Term"], how='inner')
