...
kegg_to_smiles_df = pd.read_pickle(
    join(CURRENT_DIR, "..", "data", "processed_data", "brenda", "Brenda_kegg2smiles.pkl"))
...
chebi_to_smiles_df = pd.read_pickle(
    join(CURRENT_DIR, "..", "data", "processed_data", "brenda", "Brenda_chebi2smiles.pkl"))
...
brenda_enz_sub = brenda_enz_sub.groupby(['Uni_SwissProt', 'molecule_ID'], as_index=False).apply(
    lambda x: x[x['standard_value'].notna()].head(1) if x['standard_value'].notna().any() else x.head(1)).reset_index(
    drop=True)
...
brenda_enz_inh.loc[brenda_enz_inh['Ki_value'].notna(), 'standard_type'] = 'Ki'
...
brenda_enz_inh['standard_value'] = np.where(brenda_enz_inh['Ki_value'].notna(), brenda_enz_inh['Ki_value'],
                                            brenda_enz_inh['IC50_value'])
brenda_enz_inh.drop(['Ki_value', 'IC50_value'], axis=1, inplace=True)

brenda_enz_inh.loc[brenda_enz_inh['standard_type'] == "IC50", 'standard_value'] *= 0.50
...
brenda_enz_sub_no_km = pd.merge(brenda_enz_sub_no_km, experimental_df, left_on=['Uni_SwissProt', "GO_ID"],
                                right_on=['Uniprot ID', "GO Term"], how='inner')
brenda_enz_inh_no_kiic = pd.merge(brenda_enz_inh_no_kiic, experimental_df, left_on=['Uni_SwissProt', "GO_ID"],
                                  right_on=['Uniprot ID', "GO Term"], how='inner')
