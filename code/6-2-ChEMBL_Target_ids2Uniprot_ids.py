...
chembl_binding_activities = pd.read_pickle(join(CURRENT_DIR, "..", "data", "processed_data", "chembl", "6-1-chembl_binding_activities.pkl"))
...
def map_target_ids_to_uniprot_and_go_ids(unique_target_ids):
                            ...
                            for xref in component['target_component_xrefs']:
                                if xref.get('xref_src_db') == 'GoFunction':
                                    uniprot_id_map[target_id].append(
                                        (component['accession'], component_syn['component_synonym'],xref['xref_id'],component['component_description'],xref['xref_name']))
                                    print(
    ...
    for key, value in uniprot_id_map.items():
        for item in value:
            uniprot_ec_data.append({'target_chembl_id': key, 'Uniprot_ID': item[0], 'EC_ID': item[1], "GO_ID":item[2],"component_description":item[3],"GO_name":item[4] })
...
# chembl_binding_activities.to_pickle(join(CURRENT_DIR, "..", "data", "processed_data","chembl", "6-2-chembl_binding_activities.pkl"))
