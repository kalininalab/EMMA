...
BindingDB_enz_inh = BindingDB[(BindingDB['standard_type'].isin(["IC50","Ki"])) &(BindingDB['standard_value'].between(0.0001, inh_KiEIc_nM))]
BindingDB_lap = BindingDB[(BindingDB['standard_type'].isin(["Ki",'Kd',"IC50",'EC50'])) &(BindingDB['standard_value']>= lap_KiEIc_nM)]
########################################################################################
BindingDB_enz_inh["Evidence"] = "DBA" # DBA stands for Direct Binding Assay
BindingDB_enz_inh["activity_comment"] = None

BindingDB_lap["Evidence"] = "DBA"
BindingDB_lap["activity_comment"] = None

BindingDB_enz_inh = BindingDB_enz_inh[['Uni_SwissProt', 'molecule_ID', 'Protein_Sequence','standard_type','standard_value', 'SMILES','EC_ID','Evidence','activity_comment']]
BindingDB_lap = BindingDB_lap[['Uni_SwissProt', 'molecule_ID', 'Protein_Sequence','standard_type','standard_value', 'SMILES','EC_ID', 'Evidence','activity_comment']]
BindingDB_enz_inh.reset_index(drop=True, inplace=True)
BindingDB_lap.reset_index(drop=True, inplace=True)
BindingDB_enz_inh.to_pickle(
    join(CURRENT_DIR, "..", "data", "processed_data", "bindingDB", f"9-BindingDB_enz_inh.pkl"))

BindingDB_lap.to_pickle(
    join(CURRENT_DIR, "..", "data", "processed_data", "bindingDB", f"9-BindingDB_lap.pkl"))

print(data_report(BindingDB_enz_inh))
print(data_report(BindingDB_lap))